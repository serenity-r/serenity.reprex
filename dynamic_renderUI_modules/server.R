server <- function(input, output, session) {
  ...
  observeEvent(input$dropzone, {
    # Adding new item
    purrr::map(setdiff(input$dropzone, names(item_modules)), ~ { item_modules[[.]] <- callModule(module = itemMod, 
                                                                                                 id = .,
                                                                                                 item_change = reactive({input$dropzone_selected}),
                                                                                                 one_values = one_inputs_to_reactives())})
